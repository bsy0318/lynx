...
#if ENABLE_TESTBENCH_RECORDER
bool IsValidRecorderParams(const char* params_json) {
...
extern "C" uint64_t LynxTestBenchRecorderRecordInvokedMethodWithGeneration(
  ...
  if (!IsValidRecorderParams(params_json)) {
    return 0;
  }
  ...
  recorder.RecordInvokedMethodDataFromJson(
      module_name, method_name, params_json, record_id, recording_generation);
...
extern "C" bool LynxTestBenchRecorderRecordCallbackWithGeneration(
  ...
  recorder.RecordCallbackFromJson(module_name, method_name, params_json,
