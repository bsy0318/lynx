...
TEST(TestBenchBaseRecorder, CompressToBase64String) {
  ...
  large.append("tail");
  EXPECT_EQ(ZlibDecompress(DecodeBase64(CompressToBase64String(large))), large);
}
...
TEST(TestBenchBaseRecorder, WriteRecordJsonRoundTrip) {
  ...
  const std::string path = "/tmp/lynx_recorder_write_record_json_roundtrip";
  std::remove(path.c_str());
  ASSERT_TRUE(WriteRecordJson(path, doc));
  ...
  EXPECT_FALSE(
      WriteRecordJson("/nonexistent_dir_for_lynx_ut/sub/file.json", doc));
}
...
TEST(TestBenchBaseRecorder, EndRecordOutput) {
  ...
  for (size_t i = 0; i < filenames.size(); ++i) {
    ...
    EXPECT_EQ(filenames[i], "/tmp/" + std::to_string(expected_id) + ".json");
