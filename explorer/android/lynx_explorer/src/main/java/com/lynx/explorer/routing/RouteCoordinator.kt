...
object RouteCoordinator {
  ...
  fun open(context: Context, url: String, runtime: RequestedRuntime = RequestedRuntime.AUTOMATIC, source: RouteSource = RouteSource.NATIVE_MODULE): RouteResult {
    ...
    val descriptor = try { parser.parse(url, runtime, source) } catch (error: RouteException) { return RouteResult.failure(error.code, error.message) }
...
class RouteCoordinatorEngine(
  ...
  fun open(url: String, runtime: RequestedRuntime, source: RouteSource): RouteResult {
    val descriptor = try { parser.parse(url, runtime, source) } catch (error: RouteException) {
