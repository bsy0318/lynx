...
- (void)parse:(NSArray*)actionList
  ...
  _recordedActionList = actionList;
  _recordedInvokedMethodData = invokedMethodData;
  _recordedCallbackData = callbackData;
...
- (NSString*)materializedScriptURLForURL:(NSString*)url {
  ...
  if (self.materializedScriptDirectory.length == 0) {
    NSString* rootDirectory = [cachePath stringByAppendingPathComponent:@"lynx-recorder-preloads"];
...
- (void)request:(LynxResourceRequest*)request onComplete:(LynxResourceLoadBlock)callback {
  ...
  NSString* recordedKey = [self recordedKeyForURL:url inMap:self.scriptMap];
  NSData* recordedScript = recordedKey != nil ? self.scriptMap[recordedKey] : nil;
  ...
  if (_urlRedirectMap != Nil && [_urlRedirectMap objectForKey:url] != Nil) {
    ...
    recordedKey = [self recordedKeyForURL:url inMap:self.scriptMap];
    recordedScript = recordedKey != nil ? self.scriptMap[recordedKey] : nil;
    if (recordedScript.length > 0) {
...
- (void)fetchTemplate:(LynxResourceRequest*)request
  ...
  LynxTemplateBundle* recordedBundle = recordedKey != nil ? self.scriptBundleMap[recordedKey] : nil;
